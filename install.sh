...
# config dir under ~/.memvault/config/<name>/, and the launchd service
...
install_plist() {
  ...
  local out="$PLIST_DIR/com.memvault.$VAULT_NAME.$label_suffix.plist"
  sed -e "s|__VAULT_NAME__|$VAULT_NAME|g" \
...
Generated by memvault/install.sh - re-run the installer to regenerate.
...
memvault repo (or a fork someone forgot to make public). The file was
...
log "Done. Services running: com.memvault.$VAULT_NAME.{$SERVICES}"
echo "Logs: $LOG_DIR"
